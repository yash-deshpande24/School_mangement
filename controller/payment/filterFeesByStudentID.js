...
const FilterFeesByStudentId = async (req, res) => {
    try {
        ...
        // Filter by studentId if provided in URL params
        const { studentId } = req.params;
        if (studentId) {
            allPendingFees = allPendingFees.filter(pendingFee => pendingFee.StudentID === parseInt(studentId));
            if (allPendingFees.length === 0) {
                return res.status(200).json({ message: "No pending fees found for the specified studentId criteria" });
            }
        }
...
module.exports = FilterFeesByStudentId;
