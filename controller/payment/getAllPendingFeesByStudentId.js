...
const getAllPendingFeeByStudentID = async (req, res) => {
    try {
        ...
        const pendingFees = await PendingFees.findAll({ where: { clientId: storeClientId, StudentID: studentId } });
        return res.status(200).json({ pendingFees });  
    } catch (err) {
        return res.status(500).json({ message: err.message });
    }
}
module.exports = getAllPendingFeeByStudentID