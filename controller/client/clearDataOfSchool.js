...
const deleteWholeDataByClientId = async (req, res) => {
    try {
        ...
        const deleteFeeType = await FeeType.destroy({ where: { clientId: clientId, FeeName: { [Op.ne]: "Tution Fee" } }});
        ...
        const deleteUserInfo = await UserInfo.destroy({
          where: {
            clientId: clientId,
            role: {
              [Op.and]: [
                { [Op.not]: 'Admin' }, // Exclude 'Admin'
                { [Op.or]: [ 
                  { [Op.eq]: 'Student' }, // Include 'Student'
                  { [Op.eq]: 'Parent' }, // Include 'Parent'
                  { [Op.eq]: 'Teacher' } // Include 'Teacher'
                ]}
              ]
            }
          }
...
module.exports = deleteWholeDataByClientId


// const tokenClientID = await OAuthToken.findAll ({
        //     where: {  
        //         client: {
        //             id: clientId 
        //         }
