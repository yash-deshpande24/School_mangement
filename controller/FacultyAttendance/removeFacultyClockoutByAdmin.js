...
const removeFacultyClockoutByAdmin = async (req, res) => {
    try {
        ...
        await findAttendance.update({ clockOut: null });
        return res.status(200).json({ message: 'Clocked out removed successfully' });
    } catch (err) {
        return res.status(500).json({ message: err.message });
    }
}
module.exports = removeFacultyClockoutByAdmin
